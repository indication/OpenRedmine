...
public class RedmineConnection {
	...
	public static RedmineConnection getByCursor(Cursor c){
		...
		item.setToken		(c.getString(c.getColumnIndex("token")));
		item.setAuth		(c.getInt	(c.getColumnIndex("auth")) != 0);
		item.setAuthId		(c.getString(c.getColumnIndex("authid")));
		item.setAuthPasswd	(c.getString(c.getColumnIndex("authpass")));
		item.setPermitUnsafe(c.getInt	(c.getColumnIndex("permitunsafe")) != 0);
		item.setCertKey		(c.getString(c.getColumnIndex("certkey")));
		return item;
	}
}
