...
public class SelectIssueTaskAll extends SelectDataTask<Void,Integer> {

	protected DatabaseCacheHelper helper;
	protected Long project_id;
	protected Integer filter_id;
	protected RedmineConnection connection;
	public SelectIssueTaskAll(DatabaseCacheHelper helper, RedmineConnection con, long proj){
		this.helper = helper;
		this.project_id = proj;
		this.connection = con;
	}
	public SelectIssueTaskAll(DatabaseCacheHelper helper, RedmineConnection con, Long proj, int filter){
		this.helper = helper;
	...
	protected RedmineFilter getFilter(RedmineFilterModel mFilter){
		...
		try {
			if(filter_id == null){
				...
				filter = mFilter.fetchByCurrent(connection.getId());
				if(filter == null){
					filter = mFilter.generateDefault(connection.getId());
